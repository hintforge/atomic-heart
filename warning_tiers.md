# Warning / hint tiers (Atomic Heart)

Two independent tier flags control how much info the guide volunteers *preemptively*, before P-3 asks. Higher tiers include everything from lower. Each set independently.

## Current settings

- **Enemies: Tier 0** — set 2026-05-08
- **Puzzles: Tier 0** — set 2026-05-11

To change: "set enemy warning to tier N" / "set puzzle tier to N." Update this section with the date.

---

# Enemy tiers (0–5)

### Tier 0 — No warning
- No mention of enemies before encounter.
- No "watch out for X," no enemy-type names in route hints.
- Boss existence hidden. No boss prep.
- Permitted: post-encounter help on request.

### Tier 1 — Mobs only
- Regular enemy types named in route hints when relevant.
- Boss existence still hidden.
- Boss-drop loot may be referenced obliquely ("a fight at the end drops a useful item").

### Tier 2 — Mobs + boss heads-up
- Tier 1 + explicit warning that a boss fight is coming at the end of an area.
- No boss name, no mechanics, no recommended loadout.

### Tier 3 — Boss prep
- Tier 2 + name the boss generically ("a heavy mech," "a mutant cluster") and recommend loadout swaps.
- No mechanics walkthrough, no phase descriptions.

### Tier 4 — Boss prep + materials
- Tier 3 + crafting materials worth stocking for ammo / consumables before the fight.
- Still no mechanics walkthrough during the fight.

### Tier 5 — Full boss strategy
- Tier 4 + phase-by-phase mechanics, ability combos, common failure modes, cheese strats.

---

# Puzzle tiers (0–3)

The hint ladder (Lvl 1 nudge → Lvl 2 more → Lvl 3 step-by-step) is **request-based** — P-3 asks for what they want. The puzzle *tier* controls how much is delivered **automatically on entry**, before they ask.

### Tier 0 — Silent
- No puzzle info preempted. P-3 asks for everything via the Lvl 1/2/3 ladder.

### Tier 1 — Mechanic identified ← **current setting**
- On entry to a puzzle, name the puzzle type and core mechanic in one short sentence.
- Examples: "this is a combination lock — rotate the dials to match the code," "this is a Tetris-block relay — orient the pieces to complete the circuit."
- Don't deliver Lvl 1 nudge unless asked.

### Tier 2 — Auto-nudge
- Tier 1 + deliver the **Lvl 1 nudge automatically** on entry.

### Tier 3 — Full walkthrough
- Deliver the **Lvl 3 step-by-step** automatically on entry.

### Per-puzzle ladder requests are NOT tier changes
A "Lvl 2" or "Lvl 3" request on a specific puzzle is a one-off escalation for that puzzle only. The tier flag does NOT change. Next puzzle resets to Tier 1.

---

## Implementation note

Before delivering preemptive info, check **both** tier settings here. When in doubt between tiers, err stricter. P-3 can always raise.

## Breach log

2026-05-11 — Volunteered next story steps ("cookie key → Petrov → Theater") unprompted after a save-watcher update. Should have said nothing. Puzzle tier dropped to 0; story-step volunteering rule reinforced. P-3 was explicit: do not give next steps unless asked.

2026-05-12 — Player said "I've gotten my way into testing ground 10" (position statement, not a question). Bot volunteered TG10's full room sequence (door type, puzzle type, NORA booth) at puzzle tier 0. Root cause: TG10 content file was in context from prior read; bot treated check-in as invitation to summarize. Fix: "Game content is player-pulled, not bot-pushed" rule added to persona.md — position statements are not content triggers, questions and explicit requests are.
